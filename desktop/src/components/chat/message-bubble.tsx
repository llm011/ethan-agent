...
function MessageBubbleInner({ msg, isStreaming, isLast, sessionId, onQuote, onCardAction, onRead, onShare, annotations }: MessageBubbleProps) {
    ...
    <div className={`group flex ${msg.role === "user" ? "justify-end" : "justify-start"} gap-2`}>
      {msg.role === "assistant" && (
        <div className="flex-shrink-0 mt-1">
          <img src={`${''}/logo-avatar.png`} alt="Ethan" className="rounded-full" />
        </div>
      )}
