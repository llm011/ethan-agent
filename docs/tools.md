...
`ui_card` 让 Agent 把结构化信息（对比 / 状态 / 进度 / 统计 / 列表 / 带按钮的小表单）
渲染成 [A2UI v0.9.1](https://a2ui.org/specification/v0.9.1-a2ui/) 卡片，比纯文字分点更直观。

- 参数 `messages` 是一组 A2UI envelope（`createSurface` / `updateComponents` / `updateDataModel`）。
- 工具做轻量校验（每条恰含一个 envelope 键、存在 `id:"root"`），把 envelopes 透传给前端，
  给模型只回一句简短 ack——不把整坨 JSON 回灌上下文。
- `fast_path=False`：默认不进 fast 档广播，按需经 `find_tools` 激活，避免膨胀 system prompt。
- 渲染：Web 用官方 `@a2ui/react`；REPL/终端走 `ethan/interface/a2ui_text.py` 的文本降级渲染（Panel + 邻接表还原）。
- 格式细节、组件清单、示例放在按需读取的 `ui-card` skill（`skill_read('ui-card')`），不常驻 prompt。
...
工具重写 `consent_check(**kwargs) -> str | None`：返回非空字符串表示需要授权（字符串是给用户看的操作说明），返回 `None` 放行。当前声明授权的工具：

| 工具 | 触发条件 | 说明文案 |
|------|---------|---------|
| `shell` | 任意命令 | 执行 shell 命令 |
| `file_write` | 任意写入 | 写入文件 {path} |
| `file_read` | 仅当路径在 `~/.ethan/.secrets/` 内 | 读取密钥文件 {path} |
| `get_secret` | 任意密钥读取 | 读取密钥: {name} |
...
同一会话内某工具授权过一次，后续同工具调用直接放行、不再重复询问（按 `session_id + 工具名` 粒度）。Web 与 TUI 均生效——Web 由 `WebConsentProvider(session_id=...)` 携带，TUI 在 REPL 主循环里把当前 `session.id` 绑到 provider 上，`/new`、`/resume` 切换会话时同步更新作用域。会话删除时调 `clear_session_grants` 清除。
