...
> 把 Coze Desktop 的浏览器控制链路核心逻辑迁移到 ethan,实现「不管在哪个渠道对话,都能操作 ethan server 那台机器上的浏览器(前提是装了扩展)」。
...
- 复用 Coze 扩展里成熟的 CDP / AX 快照 / ref / session 逻辑,只重写传输层。
...
| `coze-browser` CLI | **丢弃** | ethan 工具替代 |
| Desktop `browser-control/` server | **丢弃** | ethan `BrowserHub` 替代 |
| `native-host/` + 安装脚本 | **丢弃** | WS 直连,无 native host |
| `desktop-shared` 协议类型 | **移植为参考** | 在 Python 侧重定义协议常量 |

## 3. 目录结构

### 扩展(独立目录,建议与 ethan 仓库分开或放 `extension/`)
```
ethan-browser-extension/
  src/background/
    ax-snapshot.ts        # 复用 Coze
    page-controller.ts    # 复用 Coze
    cdp-client.ts         # 复用 Coze
    ref-store.ts          # 复用 Coze
    page-runtime.ts       # 复用 Coze
    session-store.ts      # 复用 Coze(调整 ownership)
    rpc.ts                # 复用方法分发,改为从 WS 收发
    ws-client.ts          # 新增:WS 连接 + 保活 + 重连
    index.ts              # 重写:启动 WS,不再 connectNative
  src/options/            # 新增:填 server 地址 + token
...
ethan/browser/
  ...
  protocol.py       # JSON-RPC method 常量 + 参数类型(对齐 Coze browser.*)
...
- **鉴权**:WS 连接首帧携带 ethan 现有 token(扩展 options 里配置)。校验失败直接 close。
- **协议**:JSON-RPC 2.0,method 对齐 Coze 的 `sessions.* / tabs.* / pages.*`。ethan 侧按 id 配对请求与响应。
...
> Coze 用的是 Native Messaging port(有「port 在即保活」语义),**完全没踩过 WS+MV3 这个坑**,这部分是净新增代码、净新增风险。
...
### 三个工具(对齐 Coze 三组命令,用 action 参数收敛)
...
## 7. 协议(JSON-RPC,对齐 Coze)

ethan BrowserHub 与扩展之间沿用 Coze 的方法命名空间,减少扩展侧改动:
...
2. **移植扩展 CDP 核心**:把 Coze 的 ax-snapshot / page-controller / cdp-client / ref-store / session-store / page-runtime 接到新的 WS 收发上,跑通 `sessions.create` + `pages.snapshot`。
...
8. **SKILL.md**:迁移 Coze 的 use-coze-browser 技能,改为 ethan 工具语义。
