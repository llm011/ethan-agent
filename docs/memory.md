...
| Episode | session 摘要 + 关键词（需求挖掘/Web 展示用） | `episodes.json` |
...
写路径（每轮对话后 _maybe_consolidate）:
  每 5 轮: StructuredMemoryExtractor → candidates → AdmissionPolicy → memories
  每  轮: Episode 写入（0 LLM）
  每10轮: collect_signals → daily/*.jsonl（做梦的原料）
  agent主动: memory_write → explicit 候选 → 立即准入
            profile_update → user_profile.md    procedure_write → playbook.json

夜间统一沉淀（0 点,run_nightly_consolidation,做梦与每日沉淀合并）:
  ① 结构化每日沉淀:当日 session 重提取 → 准入 → pending 跨 session 复评
    → TTL 过期 → general/companion 分域日摘要
  ② 重建 memory 向量索引（自愈漂移）
  ③ 做梦:同步最新 memories 为向量去重底库 → 信号 LLM 精炼 → embedding 去重
    → insight 入向量库;repetition/error 反写为候选走准入,success_path → playbook

读路径（唯一入口,每次对话构建 system prompt）:
  <memory_context>  ← build_structured_recall:FTS5/LIKE 精确通道
...
做梦（`daily_consolidation.py`）：把跨 session 的模式（重复需求 ≥3 次 /
失败教训 / 成功路径）精炼为 insight 向量条目（永久保留，不参与 LRU 淘汰），
repetition/error 反写为 inferred 候选走准入、success_path 进 playbook。
...
- **Episode**（`episodes.json`）：每轮 user_turns≥2 即写（0 LLM），
  供 heartbeat 需求挖掘与 Web API 展示
