...
- **User Profile**（`user_profile.md`）：7 个固定 section 的手写画像层，
  ...
  头像不进画像文档，作为用户级资产存在 `assets/images/_profile/img_avatar.*`
  （借 `_profile` 这个「伪 session」目录挂在 images 树下，才能被
  `/api/assets/images/{session_id}/{filename}` 服务到；放 images 根目录会 404）
