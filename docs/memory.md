...
> **为什么叫"做梦"**：人脑在睡眠时整理白天的碎片记忆、固化重要洞察、丢弃噪音。Ethan 每晚 0 点做同样的事——把白天跨 session 采集的原始信号（重复模式 / 错误 / 成功路径）精炼、去重、写入永久记忆。这个夜间沉淀流程在文档和代码注释中统一称为"做梦"（dream），对应的 `run_daily_consolidation()` 是它的实现入口。
...
### "做梦"流程 (daily_consolidation.py)

**触发**：heartbeat 的 `_midnight_loop()` 每晚 0 点执行 `run_daily_consolidation()`——即"做梦"
...
last_accessed REAL               -- 最后被 search 命中的时间（活跃度观察；不再作为淘汰依据）
```

**条目类型**：
- `insight_*`：每晚"做梦"沉淀的洞察（type=repetition/error/success_path），**永久保留**，不参与 LRU 过期
- `fact_sync_*` / `playbook_sync_*`：facts.json/playbook.json 的镜像（type=fact_sync），每次"做梦"前全量重建，不参与 LRU

**去重阈值**：`L2_DEDUP_THRESHOLD = 1.1`（L2 距离，对归一化向量）。实测该值能准确拦截同义重复，同时放行真正的新洞察。

**永久保留策略**：第五层定位是"永久记忆"，因此 `insight_*` 和 `fact_sync` 两类条目均豁免 LRU 清理——"做梦"沉淀下来的洞察不会被自动删除。`last_accessed` 字段仍会被召回时刷新，用于观察洞察的活跃度，但不作为淘汰依据。memory.db 的体量天然可控（每条洞察 ≈ 1.5KB，万条仅 ~15MB），无需靠删除控制膨胀；真正会膨胀的 sessions.db 已由独立的轮转归档机制处理。`cleanup_expired()` 作为安全阀保留，仅清理未知类型的孤儿条目。
...
| `/memory/consolidate` | POST | 手动触发一次"做梦"（测试用） |
...
```python
# heartbeat.py 的 _midnight_loop() —— "做梦"的调度入口
...
`start_heartbeat()` 同时启动常规心跳循环和 midnight 循环。`_midnight_loop` 会遍历所有用户，确保多用户部署下每个人都能各自"做梦"。
...
| Daily Consolidation | `ethan/memory/daily_consolidation.py` | "做梦"（夜间沉淀）逻辑 |
