...
- `e2e-shortcut.mjs`:8 个场景(普通页 / input / textarea / select / contenteditable / shadow DOM / iframe / 滚动后)验证快捷键都开得出面板。修复前是 3/8,现在是 8/8。
- `e2e-commands.mjs`:16 项断言覆盖排序(含真实 HTML5 拖拽事件序列)、展示数量、移出并找回,以及「点击入口真的出现面板」。

两个脚本都必须 `--headless=new`(旧 headless shell 不加载 MV3 扩展,service worker 起不来),且每步自带超时。
