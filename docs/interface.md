...
4. **消息按页加载，首屏只取最近一页**。前三条治的是「单条消息过大」，但长会话的
   **条数**本身也是瓶颈：最大会话 953 条、11MB，全量返回要 1.6s。`SessionStore.load()`
   支持 `limit` / `before` 分页，`GET /sessions/{id}` 透传这两个参数并额外返回
   `has_more` / `oldest_id`：
   ...
   前端每页 30 条（`MESSAGE_PAGE_SIZE`），实测最大会话首屏从 1602ms/9.06MB
   降到 10ms/304KB（162×）。**不要把这个 `limit` 拿掉**换回全量。

   ⚠️ **流式与分页的交互**（`use-chat-stream.ts`）：流式结束/断线重连后会拉一次最新状态，
   这里必须用 `replaceTailKeepOlder(prev, page)` 而不是 `setMessages(page)` —— 否则用户
   上滚翻出来的更早几页会被这一页冲掉，体感是「刚翻上去的历史突然没了」。
   同时占位 assistant 消息要用 `makeTempId()` 拿临时 id（`tmp:` 前缀），落库后再提升为
   真实数字 id；一切「发请求 / 以 id 为主键写状态」的地方（读过程记录、标注、删除、续跑、
   `ToolTimeline` 的原始参数/结果）都要先过 `isPersistedId()`，临时 id 发出去后端会当非法参数。
   这些规则收在 `packages/shared/src/chat/history.ts`，两端共用、有单测。
...
| GET | `/files/view?path=...&session_id=...` | 内联读取已交付的图片或 MP4；支持 HTTP Range，供 Lightbox 与 Web/Desktop 视频播放器使用（cookie / 短期签名 URL） |
...
即每个对话框只能下载/预览本对话框交付过的文件，跨 session 拿路径直接请求返回 403。
