...
每次 `delegate()` 调用会落成一条真正的 Ethan 会话（`source="delegate"`），让用户在 Ethan 侧也能看到「下发给 Coding Agent 的 query + Coding Agent 的回复 + 中间工具步骤」，而不只是主对话里一行 `delegate_coding` 工具结果。

- **粒度**：每次 `delegate()` = 一条镜像 session
  - user 消息 = 下发的 query（委派一开始就落库）
  - assistant 消息 = Coding Agent 的最终回复 + tool_steps（由 sub_steps 转来）
- **同库**：走 per-user 的 `SessionStore`（`user_sessions_db_path()`），与普通 web 会话同库，侧边栏天然能列出（按 `source="delegate"` 区分）
