...
| `tab-declutter` | 整理tab\|整理标签页\|整理浏览器\|tab整理\|清理tab\|tab分组\|declutter tab | 轻量整理本机 Chrome 的 tab：去重复 → 清无用 → 按业务主题分组。全程走浏览器扩展的 `browser_tab(action="organize")`，**2 次工具调用**跑完（`user_list` 拉清单 + `organize` 打包执行）。关键约定：组名带 `🦊 ` 前缀、每组至少 2 个 tab（单 tab 组用 `ungroup_all` 解散）、飞书文档必须下沉到业务主题而非按站点归类、整理后自动折叠 TabGroup（跳过含活跃 tab 的那组）。关闭清单**执行后**随汇报呈现，不中途征询 |
