...
| `ppt-generate` | PPT\|pptx\|幻灯片\|演示文稿\|slides\|presentation | 生成原生可编辑的 .pptx。项目制逐页生成：先定大纲（含页间衔接设计）与本 deck 专属 design system（内联主题 + 字体配对），再每页一个 `pages/NN_*.json` 独立构思版式/内容/衔接，最后逐页复审单页返修。中间格式是瘦身版 PPTist schema（JSON：slideType/textType/imageType 语义 + 结构化 runs + 预设几何形状），`render_pptx.py` 渲染为 python-pptx 原生元素（文本/形状/图表/表格/OMML 公式均可二次编辑），带文字溢出检测（中西文分宽估算 + 三级判定：可缩字兜底的 warn / 缩不住必须改文案的 error，表格末行超画布直接 error；`--check --json` 可输出结构化诊断），`gen_image.py` 负责图片瀑布流（Pexels→Unsplash→AI 生图→占位）与 Iconify 图标；两个脚本都接受项目目录或单文件 deck.json。渲染完**强制走 `deliver_file` 可视化交付**（Step 9，缺此步视为任务失败——只报文字路径用户拿不到可点击的预览/下载入口）。依赖（python-pptx 等纯 Python 包）首次运行自动 pip 安装 |
| `article-to-video` | 文章转视频\|主题做视频\|URL转视频\|生成视频\|口播视频\|article to video | 把主题、文章正文、本地 Markdown/TXT 文件或公开 URL 生成带配音和动态字幕的 MP4。网页正文按不可信素材处理；Python 流水线原子缓存 Edge TTS 音频，在隔离目录完成 Open Motion 渲染与校验后才发布 H.264/AAC 成片。支持分领域视觉：金融域 K线图（布林带 + 点位标注）、黄色关键词 callouts、虚拟人立绘角色包（`presenter_gen.py` 出 prompt，GPT image 2 出图后导入资产库复用）。在 `h3-comfyui.env` 配置 ComfyUI H3 workflow 地址后可启用 MiniMax H3 动态 presenter 管线（H3 出人物动作前景，菜单/数据由确定性 clean plate 恢复）。失败重跑会归档旧正式产物，避免误交付；最终用 `deliver_file` 交付可在 Web/Desktop 直接播放和下载的 MP4，以及制作文件 ZIP。可选媒体依赖首次使用时懒安装，不进入 `pyproject.toml` |
| `qcc` | 企查查\|qcc\|工商信息\|股东\|实控人\|失信\|被执行\|尽调\|招投标查询\|企业画像 | 企查查智能体数据平台（agent.qcc.com）实时企业数据：工商/风险/知产/经营/董监高/法规/司法案例/标讯。`qcc.py` 零依赖直连 MCP（tools/list 发现工具、tools/call 查询），鉴权读 `~/.ethan/.secrets/qcc-api-key`；对带 searchKey 的查询自动文件缓存到 `~/.ethan/data/qcc/<公司名>/<tool>_<日期>.md`，30 天过期自动重查（接口按积分计费且同主体每自然月有上限）。企业事实类问题禁用模型记忆作答，走本技能 |
| `tab-declutter` | 整理tab\|整理 tab\|整理标签页\|整理 标签页\|整理浏览器\|tab整理\|tab 整理\|清理tab\|清理 tab\|tab分组\|tab 分组\|标签页整理\|整理一下标签页\|declutter tab | 轻量整理本机 Chrome 的 tab：去重复 → 清无用 → 按业务主题分组。全程走浏览器扩展的 `browser_tab(action="organize")`，**通常 2 次工具调用**跑完（`user_list` 拉清单 + `organize` 打包执行）。关键约定：组名带 `🦊 ` 前缀 + 真实业务名（不是类型名）、每组至少 2 个 tab（单 tab 组用 `ungroup_all` 按 groupId 解散）、飞书文档必须下沉到业务主题而非按站点归类、整理后自动折叠 TabGroup（跳过含活跃 tab 的那组）。关闭清单**执行后**随汇报呈现，不中途征询。与 `use-browser` 分工：后者只管单 tab 常规操作，批量整理/清理/分组一律走本技能（trigger「浏览器」会同时命中两者，规则不得互相打架） |
