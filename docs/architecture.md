...
系统内部维护任务，启动时作为后台 asyncio 任务运行，每 N 分钟执行一次：画像分区压缩、执行 `~/.ethan/system/heartbeat.md` 中定义的周期性任务。与用户管理的 Scheduler（APScheduler）独立运行。（长期记忆的去重与复评由结构化记忆管道的准入与夜间统一沉淀负责，见 memory.md）
...
返回 Message
   ...
   └─ 如果是普通回复 → 返回给用户
                    └─ [memory/consolidator.py] 每3轮触发记忆压缩
```
-->
...
│   │   ├── heartbeat.py      # 系统心跳（画像压缩 + heartbeat.md 任务）
│   │   └── onboarding.py     # 新用户引导
│   ├── providers/
...
│   │   ├── daily_consolidation.py  # 做梦：信号精炼 → insight → embedding 去重 → 入向量库
│   │   ├── memory_vectors.py # memory 向量索引重建（自愈漂移）+ 准入语义配对
│   │   ├── legacy_migration.py # 旧 facts.json → memories 一次性幂等迁移
│   │   ├── procedures.py     # ProcedureStore（行为准则 playbook，success_patterns 已退役）
