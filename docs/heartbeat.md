...
系统内部定期执行的维护任务：画像分区压缩，运行用户定义的周期性指令（heartbeat.md），并执行决策模式抽取。
...
| 任务内容 | 任意用户业务任务（提醒、报告等） | 系统维护：画像压缩 + heartbeat.md + 决策抽取 |
...
[每日 0 点 midnight_loop]
   ...
   └─ run_nightly_consolidation()
       ...
       ② 重建 memory 向量索引
       ③ 做梦（信号精炼 → embedding 去重 → insight / success_path / 反写候选）
```
-->
