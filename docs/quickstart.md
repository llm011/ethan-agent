...
```bash
# OpenAI 兼容 API（默认推荐：OpenAI / Gemini / OpenRouter / Ollama 等）
...
Ethan 会根据消息内容的意图信号自动选择推理路径（不看字数，详见 [routing.md](routing.md)）：

- **极简问题**（"3+5"、"现在几点"、"你好"）→ instant，零工具直答；算术和时间类连模型都不调
- **命中快捷规则或 `fast_path` 技能**（如"关客厅灯"）→ fast 路径，响应最快，可用 lite 模型
- **其余** → full 路径，完整上下文 + 全量工具兜底
