...
统一放在私有 repo：**https://github.com/llm011/ethan-memory-train-data** 的 `router/` 目录下

本目录只保留生成脚本（`gen_*.py`）、训练/评测脚本和本文档。跑训练前先把数据拉下来：

```bash
# 在 paper_work/router_dataset/ 下
git clone git@github.com:llm011/ethan-memory-train-data.git _data
ln -s _data/router/train train
ln -s _data/router/val   val
ln -s _data/router/test  test
```
...
### ppt-generate（action）— 见 `gen_ppt_generate.py`
- A 主题生成  B 要点/大纲展开  C 长md·请求在前  D 长md·请求在后（hard，考头尾截断）
- E 长md·trigger 词  F 数据汇报  G 培训教学  H 路演/发布/答辩  I 美化改版  J 自然 trigger 说法
- trigger 规避：A-D、F-I 不含 trigger 子串（脚本内断言自检）；E/J 刻意保留 ~10% 含原词样本
- 配套负例在 others 的 V-陷阱·演示：胶片摄影/海报排版/网页视频/软件 how-to/长md+总结翻译润色

### others（reject）— 拒识
- 纯无关（天气/算术/翻译/写代码/闲聊/常识）
- 贴边陷阱（论文排版≠paper、git 怎么用≠deepwiki、飞书网页登录≠lark-shared、
  胶片摄影/做视频/排版设计≠ppt-generate、长文档+总结翻译≠ppt-generate）
...
- 长 query：超过 ~119 字时头 76 字 + 尾 40 字拼接后编码（MAX_LENGTH=144），
  训练（train_lr_router.py splice_long）与运行时（ethan/skills/router.py _splice_long）必须一致。
  原因：粘贴长文档时意图句常在结尾（"把以上内容做成胶片"），纯头部截断会丢意图，
  且分类器会退化成学「粘贴过 markdown」的捷径（2026-07 实测，靠长md负例样本才暴露）。
