...
class UiCardTool(BaseTool):
    ...
    description = (
        ...
        "- compare（对比表格）：{title, columns:[列1,列2,列3,...], rows:[{values:[v1,v2,v3,...]}]}\n"
        "  【重要】columns 必须包含所有列名（含第一列），rows[].values 按 columns 顺序传所有列的值，不要用 label 字段。\n"
        "- rank（排行榜）：{title, subtitle?, items:[{name, score?, desc?}]}（序号自动生成，别自己加）\n"
        "- stats（统计指标）：{title?, metrics:[{label, value, trend?}]}\n"
        "- timeline（时间轴/行程/进度）：{title, nodes:[{title, body?:str|list, items?:list[str]}]}。\n"
        "  body 传字符串为一段正文，传字符串数组或写 items 数组为分点罗列（每个 item 是一个 bullet，像你截图里那样一条一条独立换行不挤在一起）。\n"
        "文本里换行用真换行，别写反斜杠n。\n"
