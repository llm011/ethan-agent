...
1. **去 AI 味排版**：
   ...
   - 严禁使用"1. 执行摘要"、"总结"等 AI 惯用语，保持专业感。
   - **正文开头不要再写一遍标题**：飞书文档自带标题字段，正文顶部直接从元信息 + 背景摘要 Callout 起手，绝不在正文第一行把文档标题再抄一遍。
   - **标题编号**：正文最顶层章节用 H1（一级标题），以"一、二、三…"起头体现顺序（如"一、背景与问题"）；H2 及以下一律不加编号。禁止 `## 1.` 这种阿拉伯数字加点的机翻式编号。
   - **不要用分隔线（`---`）分隔标题/章节**：靠标题层级 + 留白区分即可，通篇拉分隔线显得山寨、廉价。
2. **黄金开局**：正文顶部采用「元信息 + 背景摘要 Callout」起手式（不重复标题，详见 [文档结构](#1-文档结构设计)）。
3. **视觉纵深（每篇必有图）**：
...
正文顶部**不要再写一遍文档标题**（飞书标题字段已经有了），直接从元信息起手：
...
规则：
- 文档标题（飞书标题字段）必须回答"这篇文档是关于什么的"，避免"XXX方案v2"这种版本化标题
- **正文第一行不是标题**，是元信息；标题只出现在飞书标题字段里一次
- 摘要 Callout 限制在 **3 句以内**，点明问题、解法、收益
- 元信息一定要有，方便追溯
- **不要用分隔线（`---`）把元信息、摘要、正文隔开**，留白 + 层级足够了
...
**章节命名策略**：
- **H1**：正文最顶层章节标题，以"一、二、三…"起头体现顺序（如"一、背景与问题"、"二、方案设计"）；用名词短语或动宾结构
- **H2**：主功能模块，用动宾或疑问句（"为什么要做 XXX"、"如何配置 XXX"），**不加编号**
- **H3**：具体子话题，用名词或短句（"配置参数说明"），**不加编号**
- **H4**：不推荐常规使用

> 注意：文档标题走飞书标题字段，正文里不再重复。正文顶层就是 H1（一/二/三…），别把文档标题当成 H1 又抄一遍。
...
2. **层级递进**：H1→H2→H3 不跳级
3. **对齐一致**：列表缩进方式统一
4. **视觉锚点**：每 500 字左右设置一个"小地标"（Callout/图/表）——**不要用分隔线（`---`）**，靠 Callout/图/表和标题层级来分段，分隔线通篇拉会显得山寨
5. **段落长度控制**：单段不超过 5 行
...
| **目录块** | 长文档（超过 5 个 H2）必须插入 | 放在元信息表格之后、第一个 H2 之前（不要用分割线包裹，靠留白隔开即可） |
...
- **AI 结构化废话**：严禁使用"1. 执行摘要"、"总结"等 AI 惯用语；正文顶部严格遵循铁律 2 的「元信息+背景摘要 Callout」起手式，**不在正文重复文档标题**；正文顶层章节用 H1 加"一、二、三"编号，H2 及以下不加编号，去掉 `## 1.` 这种阿拉伯数字编号，保持专业感。
- **分隔线滥用 (Divider Abuse)**：**不要用分隔线（`---` / `<hr>`）分隔标题或章节**。章节之间靠标题层级 + 留白区分即可，通篇拉分隔线显得山寨、廉价。分隔线仅在极特殊的语义切换（如正文与附录之间）偶尔使用一次，日常章节切换一律不用。
- **正文重复标题 (Duplicated Title)**：飞书文档有独立的标题字段，**正文第一行不要再把文档标题抄一遍**（无论是 H1 还是加大字号）。正文从元信息 + 背景摘要 Callout 直接起手，标题全篇只出现一次（在标题字段里）。
