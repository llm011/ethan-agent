...
description: |
  ...
  **定位**：收藏来自外部的内容（微信公众号文章、网页文章、视频链接等），以及检索/查看这些已收藏的内容。这是用户主要在手机上「剪藏/稍后读」的入口，沉淀大量外部输入。
  **不是**通用笔记落点——用户自己的想法/事实/项目/人物记录走内置 knowledge_* 工具（人物走 people-kb、项目文档走 work-notes）；小灵感/闪念走 flomo。

  **当以下情况时使用此 Skill**：
  (1) 收藏一篇外部文章 / 视频 / 链接：「收藏这篇」「存这个视频」「剪藏」「稍后读」（视频链接由 url-process 路由到这里）
  (2) 搜索或查看已收藏的内容：「找找我存的那篇文章」「最近收藏了什么」「看看原文」
  (3) 管理 Get笔记 里的标签/知识库、配置连接
trigger: "得到笔记|get笔记|biji|收藏文章|存文章|剪藏|稍后读|收藏这篇|存这个视频|我收藏的|存的那篇|抖音|B站|b站|视频链接|这个视频|那个视频|视频讲了|视频内容"
