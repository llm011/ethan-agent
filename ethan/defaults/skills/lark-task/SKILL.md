...
trigger: "飞书任务|创建任务|设提醒|待办|todo|任务管理|任务清单|我的任务|任务列表|分配任务|截止日期|任务提醒|工作日提醒|每日提醒|lark task|task create|+create task"
...
> Tip: read this skill's own files (e.g. `references/...`) with `lark-cli skills read lark-task <relative-path>` to keep them in sync with this CLI version. A reference to another skill (`../lark-foo/...`) uses the same command with the leading `../` removed: `lark-cli skills read lark-foo/...`.
