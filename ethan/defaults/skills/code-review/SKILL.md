...
**核心原则：简洁、口语化、直击要害。** 像同事在旁边跟你说话，不像 AI 生成的报告。

### 硬规则

1. **开头就说核心问题**，别铺垫、别绕一圈。不要写"⚠️ 潜在的阻塞退出风险"这种标题式废话。
2. **短句为主**，一句话说一件事。长句难读，拆开。
3. **口语化**——用"感觉这里应该…"、"是不是 xxx 更好些？"、"不然可能会…"这类表达，不要"建议："、"结果："这种机械标签。
4. **字多就分段**，但大多数评论应该 2-3 句话搞定。
5. **不要"❌ ✅"图标分栏**，不要表格式建议，不要 bullet list 罗列。就正常说话。
6. **协作式语气**，不是审判式。是给同事提建议，不是判卷。

### 好的评论示例

```
`teardownDeviceModule()` 是异步的，但 `before-quit` 里没拦退出，Electron 不等它跑完就直接杀进程了。

感觉这里应该先 `preventDefault()` 拦住，等 `prepareAppQuit()` resolve 之后再手动 `app.quit()`，不然清理逻辑可能跑一半就被掐了。
```

### 坏的评论示例

```
⚠️ 潜在的阻塞退出风险。在 before-quit 钩子中，当 isUpdateDownloaded() 为 false 且非 isUpdateInstallQuitting() 时，代码执行了 event.preventDefault() 的隐式等价逻辑吗？虽然这里没有直接调用 event.preventDefault()，但由于 prepareAppQuit() 内部调用了 setAppQuitting(true) 并异步执行 teardownDeviceModule()……（后面还有一大段）
```

问题：标题式开头、长句绕来绕去、一段到底不分段、像论文不像对话。
