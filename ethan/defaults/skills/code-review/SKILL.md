...
4. **跳过噪音文件**（见下方完整列表）。
5. **最多 8 轮工具调用**：1 轮拉清单 + 1 轮筛选 + 1 轮拉 diff 定位 + 2 轮读 diff + 1 轮发评论 + 2 轮余量。**超过 8 轮 = 失败**——不许继续读更多文件、不许 clone 仓库补上下文。读完 top 2 文件直接进入发评论。
...
| 有值得 review 的文件 | 选改动量最大的 2 个（脑子里再跳过 vendored 路径、纯 rename），记下文件名，进入第 3 步 |
...
> 交叉比对第 2 步选出的 top 2 文件，记住它们的 diff 起始行号。
> **`rg_search` 必须传 `path` 参数**，不传会全盘扫描导致超时。

### 第 4 步：读 diff 并找问题（2 轮，每个文件 1 轮）
