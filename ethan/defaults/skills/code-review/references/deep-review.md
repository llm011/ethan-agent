...
细 = 多镜头 + 读全上下文；准 = 强制 repro + 独立证伪；重 = 这两组动作本质是「一份代码被反复看很多遍、每条结论被反复挑战很多次」。**不要削减独立 pass 的数量**——多看多挑战是它准的来源。但「重」不等于「串行慢」：阶段 2 / 4 的独立 agent 必须用 `task_fanout` 并行扇出，墙钟时间 ≈ 最慢的那条分支，而不是所有分支之和。
...
> 这些拉取互相独立，**在一条消息里并行批量发起** shell / file_read 调用（工具执行器本身是并发的），不要一条条串行拉。
...
### 执行方式：task_fanout 一次并行扇出

用 **`task_fanout` 工具**一次调用带 3-7 个 subtask（每维度一个，按下方模板填 `task` 字段）。每个 subtask 在一个独立的轻量子 agent（带 shell / file_read / rg_search 只读工具）里完整执行，全部并行跑完再汇合：
- 互相看不到对方结论（子 agent 独立上下文，天然隔离）。
- 只拿一个维度 + diff / 上下文路径 + finding schema。
- 各自返回结构化 findings，汇合后进入阶段 3。

> subtask 的 `task` 必须**自包含**：材料路径（`/tmp/pr_<N>.diff`、`/tmp/pr_<N>_ctx/`、`/tmp/pr_<N>_intent.txt`）、维度关注点、finding schema 都要写全——子 agent 看不到主对话，也看不到你已读过的任何内容。
...
对每条存活 finding，用 **`task_fanout`** 一次并行派 2-3 个验证 subagent，**每个给不同反驳角度**（模板见下）。所有验证 subtask 放进**同一次** task_fanout 调用（一次最多 24 个，超出按 finding 严重度裁剪），并行跑完后按投票规则收集裁决：
...
- **工作量大是设计内的，串行不是**：阶段 2 / 4 的独立 agent 数量不要压缩，但必须全部经 `task_fanout` 并行执行（阶段 1 的拉取也要并行批量发起）。墙钟目标 ≈ 快速模式的 2-3 倍，而不是串行累加的 5-10 倍。
...
| 逐个串行跑维度 / 验证 agent（墙钟 = 分支之和） | 一次 `task_fanout` 全部并行发起（墙钟 = 最慢分支） |
