...
**规则**：
- 每个 `#` 标题是一个独立事项/方向
- `>` 引用块写路线或 checkpoint 描述（如有时间节点，同步写入 timelines.yaml）
- 列表项是当前状态/最新进展
- 标题间用 `---` 分隔
...
分类依据：根据用户记忆中的主要工作方向、或文档内容本身判断所属业务。
...
## Coze 3.0 插件架构设计
- 链接：https://xxx.feishu.cn/docx/yyy
- 一句话介绍：插件体系的整体技术方案，含 SDK 设计和沙箱隔离
- 日期：2026-07-18
```
...
当用户说「更新业务进展」「更新工作进展」「基于 xxx 更新进展」或发送文档/链接并要求更新进展时，**不要 web_search、不要 knowledge_search**，直接执行：
...
3. **写 project**：按项目写入 `~/.ethan/work/project/{项目名}.md`，格式：
   ...
   - 如有明确时间节点（DDL/checkpoint），同步写入 `timelines.yaml`
...
**注意**：这是一个**附带动作**——即使用户的主要意图是"更新进展"或"文档分析"，只要解析了链接就顺带收藏。不需要用户显式说"收藏"。
