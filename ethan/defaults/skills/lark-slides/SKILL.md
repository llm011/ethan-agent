...
| 创建失败、空白页、3350001、布局异常 | 先回读状态，再按排障清单修复，不假设原操作原子成功 | `troubleshooting.md`、`validation-checklist.md` || 把飞书 Slide 内容拿出来做离线展示 / 转 HTML / 不依赖登录态和 iframe | 回读 XML 渲染成 HTML，服务端截图权限大概率没有 | `pro/workflow/error-handling.md`「离线 HTML 渲染」、`ppt_to_html.py` |
| 多页整页重建保持原链接 | 用 `+replace-pages` 批量重建，保 `xml_presentation_id` 和原链接 | `lark-slides-replace-pages.md` |
...
**CRITICAL — 创建或大幅改写后，MUST 按 [validation-checklist.md](references/validation-checklist.md) 做显式验证：回读全文 XML、核对页数和关键元素、检查空白/破损页、明显溢出、布局风险；XML 语法和文本重叠静态检查优先使用强校验脚本 [`scripts/xml_lint.py`](scripts/xml_lint.py)（pro 增强，全面校验语法/重叠/结构），[`scripts/xml_text_overlap_lint.py`](scripts/xml_text_overlap_lint.py) 作为轻量兜底。**
...
**CRITICAL — 交付约定：生成或编辑 PPT 完成后，MUST 把该演示文稿的飞书链接（`/slides/<token>` 或 `/wiki/` 链接）一并发给用户。** 用 lark-cli 操作时从响应里取 `xml_presentation_id` 拼成 `/slides/<token>` 链接，或按用户给的原始链接原样回传。不要只交付 HTML/截图而不给飞书原链接——用户需要在飞书里打开、编辑、分享这份 PPT。若用户要求同时输出离线 HTML（预览用），则「飞书链接 + 离线 HTML」两者都给。

---

## Pro 增强能力（合并自 lark-slides-pro）

本技能已在原版基础上并入 lark-slides-pro 的增强能力，覆盖更完整的命令、更严格的校验和离线渲染路径。

### 1. 增强校验脚本（替代弱校验）

- **[`scripts/xml_lint.py`](scripts/xml_lint.py)**（203KB，pro 核心）：全量静态校验 XML——语法 well-formed、文本重叠、结构、schema 合规。**创建或大幅改写后 MUST 优先用它校验**。
- [`scripts/xml_inspect.py`](scripts/xml_inspect.py)：快速检查单页 XML 结构，定位元素。
- [`scripts/sxsd_validator.py`](scripts/sxsd_validator.py)：对 XSD schema 的专项校验。
- 原版 [`scripts/xml_text_overlap_lint.py`](scripts/xml_text_overlap_lint.py) 保留为轻量兜底。
- 使用方式：
  ```bash
  python3 ~/.ethan/skills/lark-slides/scripts/xml_lint.py <xml_file>
  ```

### 2. 离线 HTML 渲染（pro 独有，最实用的增量）

当用户要「把飞书 Slide 内容拿出来展示 / 转离线可浏览 HTML / 不依赖登录态和 iframe」时：

- **服务端截图权限 `slides:presentation:screenshot` 大概率拿不到**（scope 未授权，skill 建议不申请），像素级截图通常不可行。
- **可行路径**：回读 XML 渲染成静态 HTML，任何浏览器可看。
- **现成脚本**：[`scripts/ppt_to_html.py`](scripts/ppt_to_html.py)（无外部依赖，标准库实现）。用法：
  ```bash
  python3 scripts/ppt_to_html.py <slides.xml> <output.html>
  ```
  生成的 HTML 每页一张 slide，绝对定位还原标题/文本/表格/形状，浏览器直接可看。
- **完整排障文档**：[`pro/workflow/error-handling.md`](references/pro/workflow/error-handling.md) 末尾「离线 HTML 渲染」一节，记录了关键坑：
  - 表格坐标必须读 `topLeftX/topLeftY/width/height`（漏读会塌缩到 (0,0) 压住标题）
  - 表格列宽按 `<colgroup>` 占比分配
  - 文字容器加 `overflow:hidden` 防溢出
  - 验证必须落到浏览器截图核对，不能凭脚本输出宣布完成
  - 图标是 emoji 占位（IconPark 资源离线拿不到，要原版图形需拉 svg）
  - 标题字号 pt→px 需换算

### 3. pro 独有命令

pro 提供了若干更细粒度的命令，与原版命令并存：

| 命令 | 用途 | 文档 |
|------|------|------|
| `+add-slide` | 追加/插入单页，`--before-slide-id` 控制插入位置 | [`pro/cli/lark-slides-add-slide.md`](references/pro/cli/lark-slides-add-slide.md) |
| `+delete-slide` | 删除单页 | [`pro/cli/lark-slides-delete-slide.md`](references/pro/cli/lark-slides-delete-slide.md) |
| `+update-slide` | 整页原地覆盖更新（语义：单页整页替换） | [`pro/cli/lark-slides-update-slide.md`](references/pro/cli/lark-slides-update-slide.md) |
| `+history` | 查看演示文稿编辑历史 | [`pro/cli/lark-slides-history.md`](references/pro/cli/lark-slides-history.md) |

> 关系说明：`+update-slide`（pro，单页整页覆盖）与 `+replace-slide`（原版，块级替换/插入）互补——**单页局部编辑用 `+replace-slide`，单页整页重建用 `+update-slide`，多页批量重建保原链接用 `+replace-pages`**。三者分工清晰，不冲突。

### 4. pro 设计系统与工作流文档

并入的 pro 增强文档在 `references/pro/` 下，按需读取：

- `pro/style/`：7 类场景设计系统（商务/科技/学术/营销等）+ 兜底方案，指导整份 deck 的视觉语言。
- `pro/xml/`：XML 结构、schema 详解。
- `pro/workflow/`：
  - `slides-editing.md`：幻灯片编辑完整流程
  - `template-editing.md`：模板编辑流程
  - `validation-xml.md` / `validation-visual.md`：XML / 视觉双重验证
  - `error-handling.md`：错误码排障 + **离线 HTML 渲染**（见上）
  - `windows-compat.md`：Windows 兼容
- `pro/cli/`：pro 独有命令文档（见上）。

### 5. 能力覆盖总表

| 能力 | 原版 lark-slides | pro | 合并版 |
|------|------------------|-----|--------|
| `+replace-pages` 多页批量重建保链接 | ✅ | ❌ | ✅ |
| whiteboard 画板（SVG/Mermaid 流程图） | ✅ | ❌ | ✅ |
| 强校验脚本 xml_lint | ❌ | ✅ | ✅ |
| 离线 HTML 渲染 | ❌ | ✅ | ✅ |
| 设计系统体系 | 基础 | ✅ 完整 | ✅ 完整 |
| `+add-slide`/`+delete-slide`/`+history` | 部分 | ✅ | ✅ |
