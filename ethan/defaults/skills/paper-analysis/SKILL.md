...
## 脚本(位于 `paper-analysis/scripts/` 或包内 `ethan/defaults/skills/paper-analysis/scripts/`)

| 脚本 | 作用 |
|---|---|
| `fetch_paper.py` | arXiv ID/URL/本地路径 → PDF(仅标准库) |
| `extract_pages.py` | PDF → 逐页 PNG+文字层+manifest.json(`uv run --with pymupdf`) |
...
- **路径 B(text,通用)**:无 vision 配置 → 你直接读 `manifest.json` 每页的 `text` 字段分析。图表/公式精度有限(文字层常把表格拍平、公式变乱码),失真处诚实标注。
...
uv run --with pymupdf python scripts/extract_pages.py "<pdf>" --dpi 150  # 解析末行 manifest、effective_pages、references_start_page
```
脚本默认最多 15 页(`--max-pages N` 调),自动检测 References 起始页。末行 JSON 的 **`effective_pages`** = 实际该精读的页数(含 References 起始页、封顶 15)。
...
路径 B — 你读 `manifest` 的 `pages[N-1].text`(仅 1..effective_pages),每页一个 `file_write` 存 `analysis_page_NNN.txt`,**一轮写多页**。
