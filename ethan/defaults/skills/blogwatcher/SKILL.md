...
## Proxy Note
If you use a local proxy (e.g. `http_proxy=http://127.0.0.1:7890`), `blogwatcher-cli` has built-in SSRF protection that blocks loopback addresses. Add `--unsafe-client` to allow proxied requests:
```bash
blogwatcher-cli scan --unsafe-client
```
## Example Output
```
...
Unread articles (2):
  ...
  [2] [new] Volcano Fact
       ...
       Categories: Comics
```
## Recommended AI Blog Feeds
```bash
blogwatcher-cli add "OpenAI Blog" https://openai.com/news --feed-url https://openai.com/news/rss.xml
blogwatcher-cli add "Hugging Face Blog" https://huggingface.co/blog --feed-url https://huggingface.co/blog/feed.xml
blogwatcher-cli add "Google DeepMind Blog" https://deepmind.google/blog --feed-url https://deepmind.google/blog/rss.xml
blogwatcher-cli add "The Batch (DeepLearning.AI)" https://www.deeplearning.ai/the-batch --feed-url https://www.deeplearning.ai/the-batch/rss
blogwatcher-cli add "Lex Fridman Podcast" https://lexfridman.com --feed-url https://lexfridman.com/feed/podcast/
```
## Notes
