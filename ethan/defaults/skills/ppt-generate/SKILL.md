...
3. **高度预算**：多行文本按 layout-guide「宽度与行数估算」核一遍——中文字 ≈1 个字号宽（270px 卡片 @14px 每行只装 17 字）、行高 ≈1.32×字号、表格总高 ≈ Σ行 max(cellMinHeight, 行数×13×1.32+8) 且 `top+总高 ≤ 540`。**中文宽约西文 1.8 倍，字符数直觉不可信，必须按容器宽度换算**。
