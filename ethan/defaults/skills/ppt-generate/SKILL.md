...
2. **给这套设计语言起个名字、定一组规则**，在大纲确认时一并告知用户，例如「地图策略：纸白底 + 深海军蓝主色 + 钴蓝强调 + 朱红警示 + 路线/坐标装饰语法」。规则至少包括：
   - 主色 / 强调色（≤1 个）/ 语义色（如「Encoder 蓝、Decoder 粉」全 deck 一致）；
