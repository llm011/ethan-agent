...
Docker 镜像已内置 dida CLI，只需在宿主机授权一次，token 通过 `~/.config` 挂载自动传入容器。
...
2. **宿主机授权登录**（只需一次）：
   ```bash
   # 方式 A：浏览器 OAuth（有桌面环境）
   dida auth login

   # 方式 B：Token 方式（无桌面环境/远程服务器）
   # 在滴答清单网页版 → 头像 → 设置 → 账户与安全 → API 口令 → 创建
   dida auth token <你的TOKEN>
   ...
   授权后 token 存储在 `~/.config/dida-cli/config.json`，docker-compose 已通过 `~/.config:/root/.config` 挂载自动传入容器。
...
5. **验证**（进入容器测试）：
   ```bash
   docker exec ethan-agent dida auth status
   # ✓ 已登录

   docker exec ethan-agent dida project list --json
   # 应返回清单列表
   ```

**注意事项**：
- Token 不会过期（除非在滴答清单网页端撤销），一次授权长期有效
- 如果 token 失效，在宿主机重新 `dida auth login` 后重启容器即可
- `DIDA_ENABLED=false` 或不设置时，dida 工具不注册，不影响其他功能
