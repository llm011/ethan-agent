...
> **🚫 绝对禁止把「权限不足/未授权」当成终态。**
> 碰到任何 permission / scope / 未授权 错误（含 wiki、云盘、日历等），**不允许**只回一句「我的账号尚未授权 / 没有 wiki 权限」然后就结束本轮。
> 这种回复对用户毫无价值——用户无从下手。**必须**立刻走下面的 split-flow：发起授权、把 `verification_url`（和二维码）作为本轮最终消息发给用户。
> 即「报错 → 发链接」是一个不可分割的动作，看到权限错误就直接生成并发出授权链接，不要停在报错那一步。
