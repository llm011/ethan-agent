...
用户说"提醒我"、"每天X点"、"定时"、"待办"、"创建任务"等词时，**先判断需求类型**：

### 路径 A：飞书任务（优先）
- 用户要"设提醒"、"创建待办"、"工作日提醒我"、"分配任务给XX" → 用 `lark-task` skill 创建飞书任务
- 飞书任务原生支持提醒、重复规则、截止日期，用户可在飞书 Task 列表管理
- 工作日提醒：创建任务时设 `repeat_rule` 为工作日重复（周一到周五）
- 示例："工作日早上 9 点提醒我看邮件" → `lark-cli task +create --summary '看邮件' --due '...' --repeat_rule '...' --reminder '...'`

### 路径 B：Agent 内部调度
- 需要到点后 agent 做智能处理（查日历、生成摘要、跑分析） → 用 `schedule_create`
- 示例："每天早上 8 点给我发今日日程摘要" → `schedule_create`（agent 到点后查日历生成摘要）

### 选择原则
- 单纯提醒/待办 → 飞书任务（路径 A）
- 需要 agent 到点干活 → 内部调度（路径 B）
- 不确定时问用户："需要我到点自动处理，还是只设个飞书任务提醒你？"
