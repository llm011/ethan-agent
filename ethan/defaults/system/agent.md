...
常见 key：`defaults.max_tool_iterations`、`defaults.model`、`defaults.max_tokens`、`routing.fast_max_iters`。
api_key / auth_token / provider 不在此范围，引导用户用 `ethan provider set`。
