...
# 回环：直连宿主机（127.0.0.1）；私有网段：docker 网桥（172.17.x）、局域网（192.168.x）。
...
def _is_local(request: Request) -> bool:
    ...
    单纯检查回环会误伤合法的本地访问。加入私有网段后，公网来源仍被挡住。
...
async def chat(req: ChatRequest, request: Request, user_id: str = Depends(verify_token)):
    ...
    if req.stream:
        ...
        # 执行任意脚本（RCE）。因此强制限定：仅当请求来自本地回环或私有网段时才允许
        # 生效，公网来源一律降级为 WebConsentProvider（逐项弹窗确认）。
