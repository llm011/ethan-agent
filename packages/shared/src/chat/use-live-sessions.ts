...
/** 本 hook 只依赖这一个后端调用，由调用方注入（两端各自的 lib/api 路径不同）。 */
export interface LiveSessionsDeps {
  fetchPoll: (
    hideHeartbeat?: boolean,
    hideScheduled?: boolean,
  ) => Promise<{ sessions?: { id: string; updated_at: number }[] }>;
}
 ...
 */
export function useLiveSessions(deps: LiveSessionsDeps): void {
  // deps 常在调用方写成对象字面量，身份每帧都变。用 ref 读最新值、依赖数组留空，
  // 否则轮询会被反复清掉重建（刚建立的基线也一起丢，然后永远停在「第一次建基线」）。
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        data = await depsRef.current.fetchPoll(true, false);
      } catch {
