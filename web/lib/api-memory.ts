...
export async function wakeStructuredMemory(id: string): Promise<StructuredMemory | null> {
  const res = await fetch(`${API_URL}/memory/records/${id}/wake`, { method: "POST", headers: headers() });
...
export async function wakeScopeMemories(scopeType: string, scopeId: string): Promise<number> {
  ...
  const res = await fetch(`${API_URL}/memory/records/wake-scope?${q.toString()}`, { method: "POST", headers: headers() });
