...
import { API_URL, LLM_TASK_TIMEOUT_MS, fetchWithTimeout, headers } from "./api-base";
...
export async function fetchFacts(): Promise<Fact[]> {
  const res = await fetchWithTimeout(`${API_URL}/memory/facts`, { headers: headers() });
  if (!res.ok) throw new Error("Failed");
  return res.json().then(data => data.facts);
}

export async function deleteFact(factId: string): Promise<void> {
  await fetchWithTimeout(`${API_URL}/memory/facts/${factId}`, { method: "DELETE", headers: headers() });
}

export async function updateFact(factId: string, content: string): Promise<void> {
  await fetchWithTimeout(`${API_URL}/memory/facts/${factId}`, {
    method: "PATCH",
...
export async function fetchEpisodes(): Promise<Episode[]> {
  const res = await fetchWithTimeout(`${API_URL}/memory/episodes`, { headers: headers() });
  if (!res.ok) throw new Error("Failed");
  return res.json().then(data => data.episodes);
}

export async function deleteEpisode(id: string): Promise<void> {
  await fetchWithTimeout(`${API_URL}/memory/episodes/${id}`, { method: "DELETE", headers: headers() });
}
...
export async function fetchProcedures(): Promise<Procedure[]> {
  const res = await fetchWithTimeout(`${API_URL}/memory/procedures`, { headers: headers() });
  if (!res.ok) throw new Error("Failed");
  return res.json().then(d => d.procedures);
}

export async function deleteProcedure(id: string): Promise<void> {
  await fetchWithTimeout(`${API_URL}/memory/procedures/${id}`, { method: "DELETE", headers: headers() });
}
...
export async function fetchInsights(limit = 20, offset = 0): Promise<InsightsResponse> {
  const res = await fetchWithTimeout(`${API_URL}/memory/insights?limit=${limit}&offset=${offset}`, { headers: headers() });
  if (!res.ok) throw new Error("Failed to fetch insights");
  return res.json();
}

export async function fetchInsightsByDate(dateStr: string): Promise<Insight[]> {
  const res = await fetchWithTimeout(`${API_URL}/memory/insights/date/${dateStr}`, { headers: headers() });
...
export async function fetchTodaySignals(): Promise<Signal[]> {
  const res = await fetchWithTimeout(`${API_URL}/memory/signals/today`, { headers: headers() });
...
export async function fetchSignalsByDate(dateStr: string): Promise<Signal[]> {
  const res = await fetchWithTimeout(`${API_URL}/memory/signals/date/${dateStr}`, { headers: headers() });
...
export async function triggerConsolidation(): Promise<{ ok: boolean; added: number }> {
  ...
  const res = await fetchWithTimeout(`${API_URL}/memory/consolidate`, { method: "POST", headers: headers(), timeoutMs: LLM_TASK_TIMEOUT_MS });
...
export async function fetchStructuredMemoriesPage(params: {
  ...
  const res = await fetchWithTimeout(`${API_URL}/memory/records${recordParams(params)}`, { headers: headers() });
...
export async function searchStructuredMemories(
  ...
  if (params.domain) q.set("domain", params.domain);
  if (params.status) q.set("status", params.status);
  const res = await fetchWithTimeout(`${API_URL}/memory/records/search?${q.toString()}`, { headers: headers() });
...
export async function updateStructuredMemory(id: string, patch: StructuredMemoryPatch): Promise<StructuredMemory> {
  ...
  const res = await fetchWithTimeout(`${API_URL}/memory/records/${id}`, {
    method: "PATCH",
    headers: { ...headers(), "Content-Type": "application/json" },
...
export async function forgetStructuredMemory(id: string): Promise<void> {
  const res = await fetchWithTimeout(`${API_URL}/memory/records/${id}`, { method: "DELETE", headers: headers() });
...
export async function confirmStructuredCandidate(id: string): Promise<StructuredMemory | null> {
  const res = await fetchWithTimeout(`${API_URL}/memory/records/${id}/confirm`, { method: "POST", headers: headers() });
...
export async function wakeStructuredMemory(id: string): Promise<StructuredMemory | null> {
  const res = await fetchWithTimeout(`${API_URL}/memory/records/${id}/wake`, { method: "POST", headers: headers() });
...
export async function wakeScopeMemories(scopeType: string, scopeId: string): Promise<number> {
  ...
  const res = await fetchWithTimeout(`${API_URL}/memory/records/wake-scope?${q.toString()}`, { method: "POST", headers: headers() });
...
export async function fetchDailySummariesPage(params: {
  ...
  const res = await fetchWithTimeout(`${API_URL}${path}${suffix}`, { headers: headers() });
...
export async function triggerStructuredConsolidation(targetDate?: string): Promise<{ ok: boolean; result: Record<string, unknown> }> {
  ...
  const res = await fetchWithTimeout(`${API_URL}/memory/records/consolidate${suffix}`, {
    method: "POST",
    headers: headers(),
