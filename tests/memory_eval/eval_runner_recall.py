...
数据源: golden 集 1200 条(6 域 × 200)维护在独立仓 `llm011/ethan-memory-train-data`
的 `data/recall.jsonl`。本仓 `6c12f9e` 起只保留 runner(测试基建应跟代码同仓演进,
API drift 才会被 CI 及时抓到),数据通过环境变量定位,不进本仓。

用法:
  # 数据仓已 clone 到主仓同级目录(默认位置)
  uv run python tests/memory_eval/eval_runner_recall.py
  uv run python tests/memory_eval/eval_runner_recall.py --limit 100
  ETHAN_MEMORY_TRAIN_DATA=/path/to/ethan-memory-train-data uv run python tests/memory_eval/eval_runner_recall.py
"""
...
def _recall_jsonl_path() -> Path:
    """定位 data/recall.jsonl:优先环境变量,其次主仓同级 ethan-memory-train-data。"""
...
RECALL_JSONL = _recall_jsonl_path()
...
def seed_and_recall(case: dict, tmp: Path) -> str:
    ...
    try:
        with patch.object(paths, "CONFIG_DIR", tmp):
            ...
            result = build_structured_recall(case["query"], mode=case.get("mode", ""))
            # build_structured_recall 返回 RecallResult(非 str);取 .text
...
def main() -> int:
    ...
    ap.add_argument("--json-out", type=str, default="", help="把逐 case 结果写到 JSON 文件")
    args = ap.parse_args()

    if not RECALL_JSONL.exists():
        print(f"找不到 {RECALL_JSONL}", file=sys.stderr)
    ...
    for case in cases:
        ...
        if args.json_out:
            per_case.append({
                "id": case["id"], "domain": dom, "mode": case.get("mode", ""),
                "missed_keys": missed_keys, "leaks": leaks,
            })
    ...
    if failures:
        ...
        for x in failures[:20]:
            print("  -", x)
    if args.json_out:
        Path(args.json_out).write_text(json.dumps(per_case, ensure_ascii=False, indent=2), encoding="utf-8")
