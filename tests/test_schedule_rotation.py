...
def test_count_today_runs_counts_user_messages(tmp_path):
    """仅统计该 session 当天的 user 消息：非 user 角色（assistant/tool）、
    跨日的 user 消息、其他 session 的 user 消息均不计入。"""
    ...
    async def _run():
        ...
        other_sid = (await store.create("test-model", source="schedule", mode="")).id
        from ethan.providers.base import Message

        for _ in range(3):
            await store.save_message(sid, Message(role="user", content="run"))
        # 非 user 角色不计入
        await store.save_message(sid, Message(role="assistant", content="ok"))
        await store.save_message(sid, Message(role="tool", content="result", tool_call_id="t1"))
        # 跨日（昨天）的 user 消息不计入
        yesterday_ts = (datetime.now().astimezone() - timedelta(days=1)).timestamp()
        await store.save_message(sid, Message(role="user", content="old", created_at=yesterday_ts))
        # 其他 session 的 user 消息不计入
        await store.save_message(other_sid, Message(role="user", content="other session"))
