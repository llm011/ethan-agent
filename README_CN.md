...
Skill 从 `~/.ethan/skills/` 加载。首次运行时，包内默认技能（channels、deepwiki、lark-im、lark-shared、skills-manager、agent-browser、dev-browser）会自动复制到该目录。
...
- [x] **镜像会话**：每次 `delegate()` 落成一条真正的 Ethan 会话（`source="delegate"`），记录下发的 query + Coding Agent 回复 + 步骤，并注册为 RunManager run，委派过程可经 SSE 实时观看
